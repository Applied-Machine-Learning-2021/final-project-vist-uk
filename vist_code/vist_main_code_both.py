import argparse
from vist_methods import grab_features, vect_sentences
import torch
...
from vist_data_loader_both import get_loader
...
def main(args):
    ...
    os.chdir(r'C:\Users\javic\Documents\GitHub\AMLI_VIST_Project\vist_code\dictionaries')
    ...
    os.chdir(r'C:\Users\javic\Documents\GitHub\AMLI_VIST_Project\train_pickles') # change to train_pickles directory

    train_image_features, train_image_names, train_sentences = grab_features(train_story_data, train_story_keys)
    # vectorize sentences
    train_sentences = vect_sentences(train_sentences, in_vocab)

    # grab the image features
    os.chdir(r'C:\Users\javic\Documents\GitHub\AMLI_VIST_Project\val_pickles') # change to val_pickles directory
    val_image_features, val_image_names, val_sentences = grab_features(val_story_data, val_story_keys)
   
    # val_image_features = val_image_features[:-27]
    # val_image_names = val_image_features[:-27]
    # val_sentences = val_sentences[:-27]
    ...
    train_data_loader = get_loader(
        in_vocab, train_image_features, train_image_names, train_sentences, train_transform,
        batch_size = args.batch_size, shuffle=True, num_workers=0)

    val_data_loader = get_loader(
        in_vocab, val_image_features, val_image_names, val_sentences, val_transform,
        batch_size = args.batch_size, shuffle=True, num_workers=0)
    
    # glove directory
    os.chdir(r'C:\Users\javic\Documents\GitHub\AMLI_VIST_Project\vist_code\glove')
    ...
    total_train_step = get_length(train_data_loader)
    ...
    # if os.path.exists('models/decoder_V1-30.pth'):
    #     decoder.load_state_dict(torch.load('models/decoder_V1-30.pth'))
    #     print('decoder Model loaded')
    # if os.path.exists('models/encoder_V1-30.pth'):
    #     encoder.load_state_dict(torch.load('models/encoder_V1-30.pth'))       
    ...
    for epoch in range(0, args.num_epochs):  
        ...
        for i_step in range(0, total_train_step):
            ...
            captions_target = captions[:, 0:].to(device)
        ...
        with torch.no_grad():
            ...
            for val_step in range(0, total_val_step):
                ...
                # define the captions
                captions_target = val_captions[:, 0:].to(device)
                captions_train = val_captions[:, :val_captions.shape[1]].to(device)
        ...
        print('\r' + stats)
        #sys.stdout.flush()
        os.chdir(r'C:\Users\javic\Documents\GitHub\AMLI_VIST_Project\vist_code')
        # Save the weights.
        if epoch % 10 == 0:
            print("\nSaving the model")
